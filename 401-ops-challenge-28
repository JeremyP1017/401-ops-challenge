...
file_handler = RotatingFileHandler(log_file, maxBytes=max_log_size, backupCount=max_log_files)
...
def send_notification(from_address, to_address, password, subject, body):
    try:
        ...
        logger.error("Notification could not be sent. Error: %s", str(e))
    finally:
        server.quit()
